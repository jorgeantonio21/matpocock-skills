...
- **[ask-jorge](./skills/engineering/ask-jorge/SKILL.md)** — This fork's router: `ask-matt`'s map extended with the skills added here (`implement-by-commit`, `implement-by-plan`, `open-pr`). Reach for `/ask-jorge` day to day; `ask-matt` stays byte-identical to upstream so merges stay clean.
