...
- **The house way.** Read `CONTEXT.md` and the ADRs in the area where they exist, and read sibling files before writing a new one. Before writing Rust, call the Skill tool with "idiomatic-rust". Before writing TypeScript or changing its module or build configuration, call the Skill tool with "idiomatic-typescript". The Building tips in your context set the pace: tracer bullets, small steps, the agreed bar and no further.
