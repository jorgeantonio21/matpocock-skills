...
2. The **language idiom**: what an experienced practitioner of this language writes here. For Rust, call the Skill tool with "idiomatic-rust". For TypeScript files or TypeScript module and build changes, call it with "idiomatic-typescript". Cite the loaded language baseline's entry on each finding.
...
A documented repo standard overrides all four. Skip anything a formatter or linter enforces, and skip the bracketed Fowler overlaps: the Standards axis reports those. Where a Pragmatic tip and a language-baseline entry flag the same hunk, report it once in the language-specific form because that rewrite is more useful.
