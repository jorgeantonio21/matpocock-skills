...
It writes the agreed plan to `.scratch/<feature-slug>/commits.md` and ticks each commit off as it lands — the same `.scratch/<feature-slug>/` directory [to-tickets](https://aihero.dev/skills-to-tickets) writes local tickets into.
...
Yes. `code-review` reviews `git diff <fixed-point>...HEAD`, which excludes staged and working-tree changes — the reason it can come up empty when run before a commit exists. This skill runs it at close-out, after every commit in the plan has landed, so the whole sequence is in `HEAD` and inside the diff.
