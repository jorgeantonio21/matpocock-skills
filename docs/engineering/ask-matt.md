...
- **The main flow**, idea to ship. Grill, spec, tickets, implement (or implement-by-plan to agree the commit sequence up front, or implement-by-commit when a human reviews each commit), review, with two branches inside it: a prototype detour when a question needs runnable code to settle, and the spec-and-tickets split, which only earns its cost when the build spans more than one session.
