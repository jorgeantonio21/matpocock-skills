...
[`ask-jorge`](./skills/engineering/ask-jorge/SKILL.md) is the router that maps every user-reachable skill and how they relate. The same trigger that re-syncs a docs page applies to it: whenever you add, rename, remove, or change how a user-reachable skill fits the flows, re-read `ask-jorge`'s `SKILL.md` and update it so the map stays accurate: a new skill it never mentions, or a stale one it still routes to, is a router that lies.

[`ask-matt`](./skills/engineering/ask-matt/SKILL.md) is upstream's router, kept byte-identical to `mattpocock/skills` so upstream merges never conflict on it. Never edit it or its docs page (`docs/engineering/ask-matt.md`); route changes go in `ask-jorge`.
