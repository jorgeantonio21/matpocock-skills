---
name: open-pr
description: Ship the commits on the current branch as a pull request — one gate before anything outward-facing.
...
- Create `<type>/ja/<short-description>` at `HEAD` and switch to it — `type` read from the commits themselves (`feat`, `fix`, `refactor`, `chore`, `docs`).
