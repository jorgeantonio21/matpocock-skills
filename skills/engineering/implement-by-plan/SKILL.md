...
- Each commit is **green** on its own: checked out at that commit, the branch typechecks and its tests pass. Nobody reads these diffs before they land, so a clean checkout and `git bisect` are the whole safety net — a commit that only goes green once the next one arrives takes that net away.
...
- The plan as a numbered list — for each commit, its title, the one sentence a reviewer would use to describe it, and the files you expect it to touch.
...
- Typecheck clean.
- The tests covering this commit's behaviour pass — and the full suite, where the commit touches shared code.
...
- An order that turns out backwards — a commit needing code that lands later.
