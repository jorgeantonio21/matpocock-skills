...
3. **Branch: is this a multi-session build?**
   ...
   **Branch: the whole ticket graph?** **`/implement-spec`** walks the ready frontier of a spec's tickets with concurrent implementer subagents in separate worktrees, merges their work onto one PR branch, and closes with `/code-review` and a final fixes worker. It opens a draft PR during the run and marks it ready at close-out. Reach for it when you want the entire graph built in one run. It is in progress and installed separately.
...
Three model-invoked references supply the coding baselines beneath the build and review workflows. `/implement`, `/implement-by-commit`, and `/implement-by-plan` load `/pragmatic-programming` before implementation or planning, then the matching Rust or TypeScript baseline before writing code. `/implement-spec` passes these requirements to every code-writing worker, including final fixes and mergers that edit code. All three agents behind the pair flow carry `/pragmatic-programming`; the `implementer` and `craft-reviewer` also load the matching language baseline. Reach for a baseline directly when you want its rules applied with no flow around it. These baselines are in progress and installed separately from the promoted plugin, including when a promoted build skill needs them.
...
- **`/idiomatic-typescript`**: the framework-neutral TypeScript baseline for honest compiler and runtime guarantees. Its core covers state shape, trust boundaries, functions, mutation, errors, modules, review, and verification; disclosed references cover claimed invariants, async control flow, compiler and module tooling, and source and version evidence. The `implementer` and `craft-reviewer` read it for `.ts`, `.tsx`, `.mts`, or `.cts` diffs and TypeScript module or build changes. Reach for it yourself when writing, refactoring, or reviewing TypeScript, designing an API or validation boundary, or deciding what a TypeScript type really guarantees. React component design stays outside this framework-neutral baseline.
...
**The agents behind the pair flow**: `/full-review`, `/pair-by-commit`, and `/pair-by-plan` spawn the `implementer`, `bug-hunter`, and `craft-reviewer` agents, which live beside the skills rather than inside them. Install them into the harness before the first run (in this fork, `scripts/link-agents.sh` symlinks them). Those three skills and the three baselines are **in progress**: public on purpose, installed directly rather than through the plugin, and free to change.
