...
Three model-invoked references list rules a diff is judged against rather than supplying a workflow. All three agents behind the pair flow carry `/pragmatic-programming`; the `implementer` and `craft-reviewer` also load the matching language baseline for Rust or TypeScript. Reach for a baseline directly when you want its rules applied to the code in front of you, with no flow around it.
