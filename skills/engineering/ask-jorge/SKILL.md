...
- **`/idiomatic-rust`** (in progress): Rust invariants and contextual idioms when writing, refactoring, or reviewing Rust. It covers checked construction, admission under changing policy, persistent compatibility, and runtime/dependency choices without prescribing one architecture. The `implementer` and `craft-reviewer` agents also reach it on Rust diffs.
