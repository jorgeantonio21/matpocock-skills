...
- **`/idiomatic-rust`**: the Rust rules a linter cannot enforce, each an instruction with its reason, in the order an author decides things: Shape, Errors, Ownership, Flow, Surface, Words. Beside it sit the clippy check command it runs on every crate a diff touches, the async and hot-path idiom, a short optional set of crates that remove hand-written impls, and generic invariant patterns for validation, admission, operations, and persistence. The `implementer` and `craft-reviewer` agents read it on any diff that touches `.rs` or `Cargo.toml`. Reach for it yourself when you write, refactor, or review Rust with no pair flow around it. A documented standard in the repo overrides it.
