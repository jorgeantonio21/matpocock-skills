...
- **[ask-jorge](./ask-jorge/SKILL.md)** — This fork's router: `ask-matt`'s map extended with the skills added here (`implement-by-commit`, `implement-by-plan`, `open-pr`). Reach for `/ask-jorge` day to day; `ask-matt` stays byte-identical to upstream so merges stay clean.
...
- **[open-pr](./open-pr/SKILL.md)** — Ship the commits on the current branch as a pull request: repo checks first, a rescue off `main` onto a properly named branch, a body drawn from the diff alone, and one gate before anything outward-facing happens.
