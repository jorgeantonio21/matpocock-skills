...
- **Enums over strings.** A closed set of strings is an enum. When you see a `match` on string literals, a `&'static str` kind field, a string compared for identity, or a string constant that names a state, define an enum. Derive `EnumString` and `Display` from `strum` for the conversion at the boundary (see [CRATES.md](CRATES.md)). A `String` stays a string only for free text: a name, a message, a path from a user. The compiler then rejects a misspelled kind, and a `match` names every case.
...
- **Wire types are plain data.** Give a type that crosses the wire `#[repr(C)]` or `#[repr(transparent)]`. Name every padding byte with a field. Keep pointers, `String`, `Vec`, and `Box` out of it. Decode through a checked conversion that rejects invalid bytes, so an invalid frame never becomes a value.
...
- **Take what you use.** Take `&str`, `&[T]`, `&Path`, and `&T`. Do not take `&String`, `&Vec<T>`, `&PathBuf`, or `&Box<T>`. Take an owned value only when the function stores it or consumes it. Take `self` by value in a method on a `Copy` newtype.
...
- **Guards first, happy path flat.** Put every guard clause at the top of the function, with the reason in a one-line comment above it. Keep the happy path at the lowest indentation. Build a value that has several exit points in a labeled block, and name the label for the thing you escape.
