...
This file holds the Rust rules that a linter cannot enforce. Each rule is an instruction with its reason. To build, read the section for the work in hand, in the order the sections appear. To review, compare each changed function against every rule and name the rule on each finding. A documented standard in the repository overrides this file. The rules apply to libraries, CLI tools, inference servers, and stateful services. A rule that names a narrower case, such as the hot path, applies only in that case. The check command in [LINTS.md](LINTS.md) enforces the mechanical rules, so this file does not repeat them.

Correctness findings identify a broken guarantee. Runtime and dependency defaults depend on the project's requirements. Keep writing and readability conventions unless the repository overrides them; mark explicitly optional conventions as suggestions.

## Shape

Decide the types before you write a function body.

- **The compiler is the guardrail.** Give confusable roles their own types: `Width`, `Height`, `UserId`, `DocumentId`. Introduce a newtype when it protects an invariant, distinguishes roles, or carries useful behavior. Keep simple representations when a wrapper adds no meaningful protection. Two primitive parameters alone are not a defect; consider whether swapping them is a realistic misuse. Write `fn resize(width: Width, height: Height)`, so `resize(height, width)` does not compile. When two parameters have the same type and different roles, put them in a struct with named fields: `Move { from: FolderId, to: FolderId }`. Put a unit in the type where a unit exists: `Millis(u64)`, `std::time::Duration`.
...
- **Make invalid states impossible.** Model "one of several" as an enum. Model "can fail" as `Result`. Do not pass a `bool` argument. Use a two-variant enum such as `Visibility::Public` and `Visibility::Private`. Do not use a sentinel value such as `-1` or an empty string. Merge two fields into one enum with data when one is valid only while the other has a given value. Model a state machine as an enum, not as a set of flags. When a method is valid only in one phase, first consider module privacy and narrow methods. Use a phase type parameter when it prevents a concrete misuse those methods cannot prevent. `Connection<Handshaking>` offers `complete()`, and `Connection<Ready>` offers `send()`. Model a closed set of strings as an enum. Derive `EnumString` and `Display` from `strum` for the conversion at the boundary (see [CRATES.md](CRATES.md)). A `String` stays a string only for free text: a name, a message, a path from a user.

- **Parse at the boundary.** Convert raw input into domain types at the edge of the system, once, in one place. Raw input is bytes, JSON, a query string, or a config file. Keep validated fields private and route every entry path through checked construction. Audit constructors, conversions, parsers, deserializers, database reads, defaults, mutation, and arithmetic before claiming a guarantee. Private fields alone do not prevent construction inside the module or unchecked derives. Operations must preserve the guarantee. Give a type that is read or written as raw bytes `#[repr(C)]` or `#[repr(transparent)]`. Name every padding byte with a field. Keep pointers, `String`, `Vec`, and `Box` out of it. Decode through a checked conversion that rejects invalid bytes, so an invalid frame never becomes a value. A type that crosses a JSON or a protobuf boundary uses serde or prost and keeps its `String` and `Vec` fields.

- **Know what the type proves.** An intrinsic bound belongs to a value; an aggregate invariant relates several values. Contextual admission belongs to the authority that knows the policy. Historical acceptance does not prove continuing validity against mutable state. Preserve previously admitted work when compatibility requires it. Read [INVARIANTS.md](INVARIANTS.md) for these guarantees, authoritative transition results, shared interpretation, and persisted-data changes.

- **Newtype anatomy.** Keep the inner field private. Write `const fn new` when the supported toolchain and validation permit it. When the value has a bound, return `Option<Self>` from `new`. Use an `if` expression; `then_some` is not const-compatible on Rust 1.97.1. At a config or a request boundary, return `Result<Self, E>` instead, with the rejected value in `E`. The report then names the value. Add a `const fn new_const` that panics only when the code needs a literal of the type, such as a protocol constant. Implement `From<Newtype> for Inner` always. Implement `From<Inner> for Newtype` only when every inner value is valid. Otherwise route inbound conversion through `new` or `TryFrom`. Derive delegating `FromStr` and unchecked `Deserialize` only for unconstrained wrappers. A validated wrapper parses, then calls checked conversion; Serde can use `#[serde(try_from = "RawType")]`. Derive `Display` where delegation is correct (see [CRATES.md](CRATES.md)). Put sentinel values in associated consts: `JobId::MAX`, `Retries::ZERO`. Add `#[repr(transparent)]` when the layout matters, with a comment that says why. Write one `#[derive(...)]` line per ecosystem with a trailing comment that names the ecosystem. The comment shows a reader which surfaces the type crosses, and it stops rustfmt from merging the lines.

  ```rust
  /// An unconstrained system-generated job identifier.
...
- **Application errors use `anyhow`.** Use `anyhow::Result<T>` as a default for reporting errors in a binary or test. Follow an existing reporting convention; `Box<dyn Error>` can suffice for a small tool. When used only by tests in a library crate, `anyhow` is a dev-dependency. Add context at each layer where the lower message is not enough on its own: `.context("load the service config")?` or `.with_context(|| format!("read {path}"))?`. Do not return `anyhow::Error` from a library function. Keep actionable data in errors rather than reducing a failure to `String` or `()`. Do not mix the two conventions in one crate's public interface.

- **A module-local Result alias is optional.** Use an alias when a module's fallible operations consistently return its own error type. Qualify its definition with `std::result::Result`, or `core::result::Result` where appropriate, to avoid recursion. Match its visibility to callers; `pub` below is not mandatory. Keep explicit result types for generic errors, multiple error types, or import collisions. This convention does not require a global crate error or force tests to use the production alias.
...
- **Share by message.** Send data between tasks through a channel, or share it through an atomic or a lock-free structure (see [RUNTIME.md](RUNTIME.md)). Do not reach for `Arc<Mutex<T>>` first. On a hot path, use a lock only when no viable alternative preserves correctness. Follow the critical-section and contention requirements in [RUNTIME.md](RUNTIME.md). Elsewhere, choose synchronization from the project's requirements. Do not return a guard from a method.
...
- **Guards first, happy path flat.** Put every guard clause at the top of the function. Write the reason in a one-line comment above each guard. Write one `let ... else` per binding that exits. Write a let chain for a guard that is a condition, not an exit. Keep the happy path at the lowest indentation. Build a value that has several exit points in a labeled block. Name the label for the thing you escape.
  ...
  let removed = 'queue: {
      ...
      let Some(queue) = worker.queues.get_mut(&priority) else {
          break 'queue false;
      };
...
- **Check arithmetic on external values.** Use checked arithmetic on values that come from the wire or from a client. Map overflow to an error or a rejection. Nonzero signed values still include the minimum value, whose negation overflows. Reflect both representability and the preserved invariant in the return type. Use saturation only when the domain specifies it. Preserve valid behavior and supported exceptions when strengthening a type; do not silently round, discard, or reinterpret input. Use a plain operator only when a comment states the bound that makes it safe.
...
- **Separate logical steps.** Use blank lines between functions and between preparation, validation, and the operation that follows. Write function bodies and branching logic across lines. Format examples as readable Rust.
...
- **A type's guarantee, admission policy, or persisted representation changes.** Read [INVARIANTS.md](INVARIANTS.md).
