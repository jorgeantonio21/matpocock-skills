...
Run once per crate the diff touches, then `cargo +nightly fmt -p <crate>`. Repeat `-p` for several crates.

```bash
cargo clippy --no-deps -p <crate> --all-features -- -D warnings -W clippy::pedantic -A clippy::similar_names -A clippy::must_use_candidate -A clippy::inline_always -D clippy::unwrap_used -D clippy::expect_used -D clippy::panic_in_result_fn -D clippy::dbg_macro -D clippy::print_stdout -D clippy::print_stderr -D clippy::exit -D clippy::undocumented_unsafe_blocks -D clippy::allow_attributes_without_reason -D clippy::await_holding_lock -D clippy::large_futures -W unreachable_pub -W missing_debug_implementations -W unsafe_op_in_unsafe_fn
```
...
- `-A clippy::similar_names`: pairs such as `order_a` and `order_b` are deliberate in matching code, and the lint has no way to read intent.
...
```bash
cargo clippy --no-deps -p <crate> --all-features --message-format=json -- <the flags above> | jq -r --argjson files "$(git diff --name-only "$(git merge-base HEAD origin/main)" | jq -R . | jq -s .)" 'select(.reason=="compiler-message") | .message | select(.code != null) | select(any(.spans[]; .is_primary and (.file_name as $f | $files | index($f) != null))) | .rendered'
```
...
```bash
cargo clippy --no-deps -p <crate> --all-targets --all-features -- -D warnings -W clippy::pedantic -A clippy::similar_names -A clippy::must_use_candidate -A clippy::inline_always -D clippy::dbg_macro -D clippy::print_stdout -D clippy::print_stderr -D clippy::exit -D clippy::undocumented_unsafe_blocks -D clippy::allow_attributes_without_reason -D clippy::await_holding_lock -D clippy::large_futures -W unreachable_pub -W missing_debug_implementations -W unsafe_op_in_unsafe_fn -A clippy::unwrap_used -A clippy::expect_used -A clippy::panic_in_result_fn
```
...
Each rule below is enforced by the command and never appears in `SKILL.md` prose. A rule with no active lint (a clone to satisfy the borrow checker, `Arc::clone(&x)` spelled out, a `_` arm on a local enum, `Self` inside an impl, `mem::forget` kept out of hand-written code) stays in the skill. Level: `default` is a style, complexity, perf, or suspicious lint that is warn out of the box; `pedantic` arrives with `-W clippy::pedantic`; `pick` is a `-D` in the command; `rustc` is a `-W` in the command.
...
| Shape | Seven or fewer parameters; one hundred or fewer lines | `too_many_arguments`, `too_many_lines` | default, pedantic |
| Shape | `Result` or `Option` only where a function can fail or be absent | `unnecessary_wraps` | pedantic |
| Ownership | `&str`, `&[T]`, `&T` parameters; owned only when consumed | `ptr_arg`, `borrowed_box`, `needless_pass_by_value` | default, pedantic |
...
| Flow | `unwrap` and `expect` only in tests; elsewhere `?`, or `#[expect]` with the invariant at a provably infallible site | `unwrap_used`, `expect_used` | pick |
...
| Runtime | No `std::sync` guard held across `.await` | `await_holding_lock` | pick |
