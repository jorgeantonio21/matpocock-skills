...
- **Share without a lock.** On the hot path, a lock is the last resort. A blocked thread there misses its deadline, and a few lines inside the guard do not bound the wait: contention does. Before a lock, try these in order. Give the state one owner and send it messages. Partition the state so each thread owns a shard. Publish an immutable snapshot with `arc_swap::ArcSwap`, which many readers load and one writer replaces. Use an atomic for a flag, a counter, or one small value: `AtomicBool`, `AtomicU64`, `AtomicUsize`. Use a wait-free or lock-free structure. A wait-free operation completes in a bounded number of steps whatever the other threads do, and a lock-free operation guarantees that some thread makes progress. An SPSC ring such as `rtrb` between two dedicated threads is wait-free on push and pop. `crossbeam::queue::ArrayQueue` (bounded) and `SegQueue` (unbounded) are lock-free when several threads produce. Do not assume a channel or a concurrent map is lock-free inside. `crossbeam-channel` and `flume` take a lock or park a thread on some operations, and `dashmap` shards a lock. Read the crate's documentation for the operation you call, and keep those off the hot path unless a measurement clears them. At the edge, a short lock is a normal choice, and the order above is a preference, not a rule.

- **A hot-path lock, when nothing else fits.** Take a lock on the hot path only when one operation must read and write several fields together. Even then, only when none of the alternatives above preserves correctness. Then keep the critical section to the few lines that touch the shared state. Prepare the data, allocate, do I/O, and call callbacks outside the guard. Never hold the guard across an `.await`. Write a comment at the lock site that says why no alternative fits and how many threads contend for it. A short critical section does not bound the wait, so measure the acquisition latency under load and record the number beside the comment. Choose the mutex from the project's requirements. `std::sync::Mutex` adds no dependency and poisons on a panic. `parking_lot::Mutex` is the choice when the project already uses it, or needs the smaller guard and no poisoning. Do not add a crate for one lock.
...
- **Locks and `.await`.** At the edge, a sync mutex guards data for a few lines with no `.await` inside. `std::sync::Mutex` or `parking_lot::Mutex` is the project's choice. Do not hold its guard across an `.await`: drop the guard first, or move the work into a sync fn. Use `tokio::sync::Mutex` only when the guard must live across an `.await`, such as exclusive use of one connection. It is slower, and it is not for data. Alias the sync mutex with `use parking_lot::Mutex as SyncMutex;` when a tokio lock is in scope. On the hot path, no lock, `Notify`, or async mutex, except as "A hot-path lock, when nothing else fits" allows.
