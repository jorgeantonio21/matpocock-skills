...
- **[idiomatic-rust](./idiomatic-rust/SKILL.md)**: Concise, idiomatic, readable Rust, each rule an instruction with its reason in Simplified Technical English, in six sections: Shape, Errors, Ownership, Flow, Surface, Words. Beside it, `LINTS.md` (the clippy check command the agent runs on touched crates, no repo change), `RUNTIME.md` (async and threads), `CRATES.md` (optional crates and their tradeoffs), and `INVARIANTS.md` (generic validation, admission, operation, and persistence guarantees). Reached by the `implementer` and `craft-reviewer` agents on Rust diffs.
