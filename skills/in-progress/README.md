...
- **[idiomatic-rust](./idiomatic-rust/SKILL.md)**: Concise, idiomatic, readable Rust, each rule with its rewrite, in five sections: Shape, Ownership, Flow, Surface, Words. Beside it, `LINTS.md` (the clippy check command the agent runs on touched crates, no repo change), `RUNTIME.md` (async and threads), and `CRATES.md` (the crates that retire hand-written impls, with a skip list). Reached by the `implementer` and `craft-reviewer` agents on Rust diffs.
